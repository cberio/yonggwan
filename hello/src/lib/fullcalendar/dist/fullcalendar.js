...
function getEvX(ev) {
	...
	//수정: scroller container의 scrollLeft 값을 더함.
	var containerScrollX = 0;
	if ($('.fc-agendaDay-view').length) {
	  containerScrollX = $('.fc-agendaDay-view').parent('.fc-view-container').scrollLeft();
	}

	if (ev.pageX !== undefined) {
		return ev.pageX + containerScrollX;
	}

	var touches = ev.originalEvent.touches;
	if (touches) {
		return touches[0].pageX + containerScrollX;
	}
}
...
Grid.mixin({
	...
	bindSegHandlersToEl: function(el) {
		...
		// 수정 : event double click 이벤트 옵션 추가 (1)
	...
	// 수정 : event double click 이벤트 옵션 추가 (2)
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
		...
		// 수정 : 캘린더 상단 날짜 label 표기형식 변경  :  '31(토)'
		var customLabelFormat = htmlEscape(date.format('D')) + htmlEscape('(' + date.format('dd') + ')');
	...
	renderBgCellHtml: function(date, otherAttrs) {
		...
		return '<td class="' + classes.join(' ') + '"' +
			...
			(otherAttrs ?
				...
				'') +
			'>' +
				'<div class="fc-create-helper">' +
					'<table>' +
						this.renderSlatRowHelper() +
					'</table>' +
				'</div>' +
				'<div class="fc-bg-inner">' +
				'</div>' +
			'</td>';
	},

	// 수정: custom html 삽입 (슬롯 버튼 용 그리드)
	renderSlatRowHelper: function() {
	  var view = this.view;
	  ...
	  while (slotTime < this.maxTime) {
	    ...
	    html +=
	      '<tr data-time="' + slotDate.format('HH:mm:ss') + '"' +
	        '>' +
	        '<td class="' + view.widgetContentClass + ' fc-slot"/>' +
	      "</tr>";
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderDates: function() {
		...
		this.slatContainerEl = this.el.find('.fc-slats').not($('.fc-slats-clone'));
	...
	renderHtml: function() {
		return '' +
			...
			'<div class="fc-slats">' +
				...
				'</table>' +
			'</div>'+
			// 수정 (추가) fc-slat-clone 엘리먼트 삽입
			'<div class="fc-slats fc-slats-clone">' +
				'<table>' +
					this.renderSlatRowHtml() +
				'</table>' +
			'</div>';
	...
	// 수정 fc-slot 클래스 추가 및 슬롯 label tag wrapping 및 time format locale EN적용
				...
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
						'<span>' + ( // for matchCellWidths
							slotDate.format('HH:mm:ss') === '12:00:00' ? (
								'정오'
							) : slotDate.format('HH:mm:ss') === '00:00:00' ? (
								'자정'
							) : htmlEscape(slotDate.locale('en').format(this.labelFormat))
						) +
						'</span>' :
						''
						) +
	...
	renderNowIndicator: function(date) {
		...
		// 아래 수정 (now-indicator-line 을 expert length 만큼 삽입하지않고 하나만 삽입)

		// render an arrow over the axis
		if (segs.length > 0) { // is the current time in view?

			// render lines within the columns1
			// for (i = 0; i < segs.length; i++) {}
			nodes.push($('<div class="fc-now-indicator fc-now-indicator-line"></div>')
				.css('top', top)
				.appendTo(this.el.find('.fc-content-skeleton-clone'))[0]);
				//.appendTo(this.colContainerEls.eq(segs[i].col))[0]);

			nodes.push($('<div class="fc-now-indicator fc-now-indicator-arrow"></div>')
				.css('top', top)
				.html(
					'<span class="ampm">'+ moment(date).locale('en').format('A')+'</span><span class="time">'+ moment(date).format('h:mm') +'</span>'
				// 수정 $('.fc-content-skeleton') 이 아닌 $('.fc-content-skeleton-clone') 으로 삽입
...
TimeGrid.mixin({
	...
	renderContentSkeleton: function() {
		...
		skeletonEl = $(
			'<div class="fc-content-skeleton">' +
				...
				'</table>' +
			'</div>' +
			'<div class="fc-content-skeleton-clone"></div>'+
			'<div class="modal-mask" style="display:none;" id="render-confirm"></div>' +
      '<div class="modal-mask mask-event mask-white" style="display:none;"></div>'
		);
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var segClass = seg.event.class;
		classes.unshift(segClass);
		// ***
		...
		if (view.isMultiDayEvent(event)) { // if the event appears to span more than one day...
			...
			if (seg.isStart || seg.isEnd) {
				// timeText : ex 3:00 - 4:00
				timeText = this.getEventTimeText(seg);
				// fullTimeText : ex 오후 3시 0 - 오후 4시 0분
				fullTimeText = this.getEventTimeText(seg, 'LT');
				// startTimeText : ex 3:00
			...
			startTimeText = this.getEventTimeText(event, null, true); // displayEnd=false
		}



		return '<a class="' + classes.join(' ') + '"' + ' id="ID_'+ event.id + '"'+
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
			...
			'>' + // *수정*  이하 slot display 추가 컨텐츠
				...
				'<div class="fc-content">' +
				//임시추가
				'<div style="color:black; font-size:12px; font-weight:bold; position: absolute; left: 0; top: 0; line-height:1;">ResourceId: '+event.resourceId+'</div>'
				+
					(
						// OFF TIME 일 경우 컨텐츠
						event.status == '05' ? (
							...
							'<div class="fc-ui">'+
								'<button class="fc-ui-delete">삭제</button>' +
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					// 수정 : h2 > span tag 삽입하여 date, day 분리
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2><span class="date"></span><span class="weekDay"></span></h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
...
// NOTE: can not guarantee any of these computations will run because not every locale has datepicker
...
var AgendaView = FC.AgendaView = View.extend({
	...
	// 수정: custom 예약마감버튼 삽입
	renderDeadlineElement: function () {
		var options = this.options;
		return $('<button class="order-deadline-button" />')
			.text('예약마감')
			.on('mouseup', options.reserveDeadline); // mouseup -> click 으로 바꾸어야하지만 작동에러.. 170418
	},
	...
	renderDates: function() {
		...
		var dateOfNow = this.calendar.getNow();
		var dateOfToday = this.calendar.getDate();
		if (dateOfNow.isSame(dateOfToday, 'day')) {
			timeGridWrapEl.append(this.renderDeadlineElement());
		}
		// }
...
var ListViewGrid = Grid.extend({
	...
	handleSegDblClick: function(seg, ev) {
		...
		Grid.prototype.handleSegDblClick.apply(this, arguments); // super. might prevent the default action
