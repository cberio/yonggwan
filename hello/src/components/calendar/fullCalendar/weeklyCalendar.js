...
class WeeklyCalendar extends Component {
                ...
                $(this).on({
                    mouseenter(e) {
                        ...
                        if (component.state.newScheduleServiceTime) {
                            const className = (component.state.isEditEvent
                                ? 'shc-edit'
    ...
    bindTimelineScroller(type) {
        ...
        $(controlerElem).scroll(function () {
            ...
            const thisLeft = $(this).scrollLeft();
    ...
    autoFlowTimeline(x, y, jsEvent) {
        let timeline = $('.fc-view-container'),
            offset = timeline.offset();
        const max = {
            left: offset.left,
            right: offset.left + timeline.width()
        };
        const thisLeft = $(timeline).scrollLeft();
        if (x <= max.left + 1) {
    ...
    removeConfirm(schedule) {
        this.props.isModalConfirm('removeSchedule');
        this.setState({
            isModalConfirm: true,
    ...
    beforeInitConfirmRenderNewSchedule(bool, newSchedule) {
        ...
        const newScheduleID = this.props.returnNewID();
        const insertSchedule = $.extend(this.props.returnScheduleObj(newSchedule), {
            id: newScheduleID
        });
    ...
    checkBindedSlot(e) {
        if (this.state.isCreateOfftime)
            this.bindNewOfftime('render', 'offtime');
        else if (this.state.unknownStart || this.state.isEditEvent) {
          // ...
        } else
    ...
    fakeRenderEditEvent(editSchedule, rerender) {
        ...
        $('.create-order-wrap.timeline button.create-event').ready(() => {
            $('.create-order-wrap.timeline button.create-event').bind('click', function () {
                ...
                const insertSchedule = $.extend(editEventObj, {
                    id: editSchedule.id,
    ...
    changeDate(date) {
        ...
        if (this.checkRenderedDate(date)) {
            const scrollX = $(this.controler).scrollLeft();
            const positionX = $(`th.fc-day-header.fc-widget-header[data-date="${moment(date).format('YYYY-MM-DD')}"]`).position().left;
    ...
    newOrderCancel() {
      ...
      // 시작시간을 미리 선택하지않고 이벤트를 생성중에 취소할 경우
        if (this.state.unknownStart || this.state.isEditEvent)
          ...
          // enable editable
            const evt = $(Calendar).fullCalendar('clientEvents', this.state.newScheduleID);
            evt.editable = true;
            $(Calendar).fullCalendar('updateEvent', evt);
    ...
    eventSlotHighlight(bool, type, service) {
        ...
        const daySlot = $(`.fc-agendaWeekly-view .fc-bg .fc-day[data-date="${moment(new Date()).format('YYYY-MM-DD')}"]`);
    ...
    componentDidMount() {
        ...
        const staffsUiHeight = 38 - 17; // 17 = window scrollbar height
        ...
        $(Calendar).fullCalendar($.extend(component.props.fcOptions, {
            ...
            eventRender(schedule, element, view) {
              // 주단위 타임라인 에서 expert 별로 이벤트를 렌더링합니다. (filtering)
                ...
                const currentStaff = priorityStaff || renderedStaff || lastStaff || Staffs[0];
          ...
          // 캘린더 이벤트 view 렌더링시
            viewRender(view, elem) {
              ...
              // state를 업데이트 후에 실행해야 하는 함수
                const runAfterStatesUpdate = function () {
        ...
        if (this.props.isBindedNewOrder) {
            const newOrderInfo = this.props.getNewOrderStates();
            this.setState({
    ...
    render() {
        ...
        const CreateOrderButtonDirectProps = {
            ...
            buttonClasses: this.state.unknownStart || this.state.isEditEvent ? 'create-evnet' : '',
        };
