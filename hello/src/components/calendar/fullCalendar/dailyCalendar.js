...
class DailyCalendar extends Component {
                ...
                $(this).on({
                    mouseenter(e) {
                      ...
                      // current time setting
                        const thisService = Functions.getService(
                        component.state.newScheduleService,
    ...
    renderNewOfftime() {
        ...
        component.props.createNewSchedule(offTimeObject).then((createdOffTimeResult) => {
            ...
            const createdSchedule = createdOffTimeResult.schedules.data;
    ...
    fakeRenderEditEvent(editEvent, rerender) {
        ...
        $('.create-order-wrap.timeline button.create-event').ready(() => {
            $('.create-order-wrap.timeline button.create-event').bind('click', function () {
                ...
                // 수정된 이벤트 객체 정보
                const getEventObj = component.refs.NewOrder.getEventObj();
                const editEventObj = component.props.returnEventObj(getEventObj);
            ...
            eventClick(schedule, jsEvent, view) {
                ...
                if (!component.state.isEditSchedule)
                    ...
                    // *** 2_삭제 ***
                        ...
                        component.props.isModalConfirm('removeSchedule');
                        component.setState({
                            isModalConfirm: true
                        });
                    }
    ...
    render() {
        ...
        const StaffsInputEach = !_.isEmpty(Staffs) ? Staffs.map((staff, i) => (
          <div
        ...
        const viewstate = (
          <dl className="viewstate fc">
            ...
            <dt>isNewOrder :</dt>
            <dd>{this.props.isNewOrder ? 'true' : ''}</dd>
            <dt>viewTypeOrder :</dt>
        ...
        return (
          <div ref="Calendar" id="daily">
            ...
            {StaffsInterfaceComponent}
            {this.props.getNewOrderComponent(this, 'agendaDay')}
...
const mapDispatchToProps = dispatch => ({
    ...
    finishRequestReservation: () => dispatch(
          actions.requestReservation({ condition: false, requestEvent: undefined })
        )
});
