...
class DailyCalendar extends Component {
    constructor(props) {
        ...
        this.bindComponentChildren = this.bindComponentChildren.bind(this);
    ...
    // 다수의 Experts 를 렌더링했을 때, 각 Expert timeline의 최소 width를 적용함,
    // 각 Expert timeline의 th에 expert input 을 삽입함.
    setCalendarColumn (type, experts) {
        let {Calendar} = this.refs;
        ...
        switch (type) {
            ...
            case 'again':
                ...
                // case ************************************************ //
            case 'destroy':
                var expertsElem = $('.expert-each.checkbox');
                // Expert level 별로 재정렬한다.
                let expertsElemNew = expertsElem.detach().sort(function(a, b) {
                ...
                $(expertsElemNew).appendTo('.expert-daily .expert-inner');
        ...
        if (
            expertLength >= 2 &&
            windowWidth < (headerWidth + timeSlatWidth + (columnMinWidth * expertLength))
          ) {
    ...
    // Full Calendar 하위 ELements 가 아닌 Calendar Component 하위 엘리먼트 바인딩
    bindComponentChildren () {
    ...
    bindTimelineAccess () {
      ...
      var d = getDate.format('YYYY-MM-DD');
          // slot / button variabling
      var createButtonElem = $('.create-order-wrap.timeline'),
          createHelperSlot;
      var getExperts = $(Calendar).fullCalendar('getResources');
          ...
          $(parentElem).find('.fc-slot').each(function() {
              $(this).on({
                  mouseenter: function(e) {
                      ...
                      component.setState({selectedDate: selectedTime});

                      // get selected expert
                      if ($(Calendar).fullCalendar('getResources').length === 1) {
                          component.setState({
                              selectedExpert: component.props.getExpert($('.expert-each.checkbox input:checked').val())
                          });
                      } else {
                          component.setState({
                              selectedExpert: component.props.getExpert($(this).parents('.fc-day').data('resource-id'))
                          });
                      }
    ...
    toggleCreateOrderUi() {
        ...
        if (this.state.renderedExpert.length <= 1) {
    ...
    step_confirm(bool, newEvent) {
        ...
        this.setState({
            ...
            if ($('input#expert_' + insertEvent.resourceId).prop('checked')) {
                ...
                this.renderExpert(this.props.getExpert(insertEvent.resourceId), $('input#expert_' + insertEvent.resourceId), true);
                setTimeout(function() {
                    component.fakeRenderNewEvent(insertEvent, newEventId, 'selectedStart');
                }, 100);
            }
            ...
            if (isNotAutoSelectTime) {
                if (this.state.viewTypeOrder === 'agendaDay') {
                    ...
                    this.changeView('agendaDay', function() {
                        // render event
                        component.autoScrollTimeline($('#ID_' + id));
    ...
    createOfftime(order, type) {
        ...
        switch (order) {
            ...
            case 'weekly':
                this.setState({
                    isCreateOfftime: true, newEventProductTime: 20,
    ...
    newOrder(type) {
        ...
        this.setState({viewTypeOrder: 'agendaDay'});
    ...
    expertInputCheck () {
        ...
        if (priorityExpert) {
            ...
            for (let i = 0; i < renderedExpert.length; i++) {
                if (priorityExpert.id !== renderedExpert[i].id)
                    $(Calendar).fullCalendar('removeResource', renderedExpert[i].id);
            }
        ...
        if (element.prop('checked')) {
            ...
            if (expert === 'all') {
                ...
                for (let i = 0; i < Experts.length; i++) {
                    $(Calendar).fullCalendar('addResource', Experts[i], true);// 1-2 Each Expert;
                }
            } else {
            ...
            if (expert === 'all') {
                ...
                $('.expert-each.checkbox input').each(function(i, elem) {
                    if (component.state.defaultExpert.id === $(elem).val()) {
                ...
                if (this.state.renderedExpert.length <= 2) {
                    this.setState({
                        lastExpert: this.props.getExpert($('.expert-each.checkbox').find('input:checked').val()),
          ...
          eventClick: function(event, jsEvent, view) {
              component.setState({
                  selectedEvent: $.extend(event, {
                      class: Functions.getProductColor(event.product, Products)
                  })
              });
    ...
    render() {
        ...
        const CreateOrderButtonFixed = (
            <div className="create-order-wrap fixed">
                ...
                <div className="create-order-ui">
                    <button onClick={() => this.newOrder('notAutoSelectTime')} className="ui-reservation">예약생성</button>
                    <button onClick={() => this.createOfftime('weekly')} className="ui-offtime">OFF TIME 생성</button>
                </div>
            </div>
        );
        const CreateOrderButtonTimeline = (
            <div className={`create-order-wrap timeline${this.state.isCreateOfftime
                ? " off-time"
                : this.state.isNotAutoSelectTime
                    ? " has-card"
                    ...
                    <div className="create-order-slot">
                        {this.state.isCreateOfftime
                            ...
                            : this.state.isNotAutoSelectTime || this.state.isEditEvent
                                ...
                                : (
                                    <button className="create-button" onClick={this.toggleCreateOrderUi}>
                                        <i className="time"></i>
                                        <span>+</span>
                                    </button>
                                )
                            ...
                            <div className="create-order-ui-inner">
                                <div className="create-order-ui">
                                    <button onClick={() => this.newOrder()} className="ui-reservation">예약생성</button>
        ...
        var ExpertsInputEach = Experts.map((expert, i) => {
            return (
                <div
                  className="expert-each checkbox"
                  data-id={`expert_${expert.id}`}
                  data-active="false"
                  data-priority={expert.priority}
                  key={i}>
                    <input disabled={this.state.isRenderConfirm}
                      className="expert-input"
                      type="checkbox"
                      name="expert" id={`expert_${expert.id}`}
                      value={expert.id}
                      onChange={(input) => this.renderExpert(expert, input.target)}/>
                    <label
                      className="expert-label"
                      htmlFor={`expert_${expert.id}`}>{expert.title}
            ...
            <div className="expert-wrap">
                <div className="expert-ui expert-daily">
                    <div className="expert-inner">
                        {Experts.length >= 2 && ExpertsInputAll}
                        {ExpertsInputEach}
                    </div>
                </div>
            </div>
        ...
        if (this.state.isChangeDate) {
            DatePickerComponent = (
              <DatePicker selectedDate={this.state.timelineDate}
        ...
        if (this.state.isUserCard) {
            UserCardComponent = (
              <UserCard cards={this.props.events}
        ...
        let viewview = (
            <dl className="viewview fc">
                ...
                <dd>{this.state.selectedExpert
                        ? this.state.selectedExpert.title
                        : ""}</dd>
                <dt>prevExpert</dt>
                ...
                <dd>{this.state.renderedExpert
                        ? this.state.renderedExpert.map((expert, i) => {
                            return expert.title + ","
                        })
                        : ""}</dd>
                <br/>
